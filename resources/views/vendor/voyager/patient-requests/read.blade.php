...
@section('page_header')
    <h1 class="page-title">
        ...
        <a href="{{ route('exportpatient',$dataTypeContent->id) }}" class="btn btn-primary">
            <span class="glyphicon glyphicon-download"></span>&nbsp;
            Download
        </a>

        <a href="#mail_send_modal" class="btn btn-primary" data-toggle="modal" data-backdrop="static" data-keyboard="false">
                <span class="glyphicon glyphicon-send"></span>&nbsp;
                Send
        </a>
        {{-- Mail Send Model --}}
            <div class="modal modal-primary fade" tabindex="-1" id="mail_send_modal" role="dialog">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('voyager::generic.close') }}"><span
                                            aria-hidden="true">&times;</span></button>
                                <h4 class="modal-title"><i class="voyager-send"></i>Send patient details to hospital</h4>
                            </div>
                            <div class="modal-footer">
                                <div class="container">
                                <form action="{{route('send_to_hospital',$dataTypeContent->id)}}" method="POST">
                                    @csrf
                                        <div class="form-group row">
                                            <label for="hosp">Select Hospital </label>
                                                <select name="hosp_name" id="hosp">
                                                    <option value="">Select Hospital</option>
                                                    @foreach ($hospitals as $hospital)
                                                <option value="{{$hospital->email}}">{{$hospital->name_en}}</option>
                                                    @endforeach
                                                </select>
                                        </div>
                                        <label>OR</label>
                                        <div class="form-group row">
                                            <label for="hosp">Enter email of hospital</label>
                                            <input name="hosp_name2" type="email" id="hosp">
                                        </div>
